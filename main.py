...
# agent_LASSO_greedy = Agent_LASSO(n=n, h=h, k=k, greedy_only=True, lam=0.01)
# agent_LASSO = Agent_LASSO(n=n, h=h, k=k, greedy_only=False, lam=0.01)
plt.figure()
...
for agent in agentList:
	...
	## Plot the results
	roll_mean_cum_regret = pd.DataFrame(cum_regret).rolling(10).mean() #to plot moving average for less noise
	# plt.figure()
